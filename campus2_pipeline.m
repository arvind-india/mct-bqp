...
start_frame = 4;
plotDebugBoundingBoxes(cameraListImages,allDetections,start_frame);
%%=======================================================
% Plot detections of all peds in both cameras (we consider that peds are represented by the middle of their BB's)
plotDetectionsCameraSpace(cameraListImages,allDetections);
%%=======================================================
...
% Plot pedestrians
plotDetectionsGroundPlane(cameraListImages,allDetections,homographies);
% Plot overlap of camera regions
overlap = computeIntersections(homoplanes);
%%==============================================================
...
%-------------This is done for every frame
    disp(['Computing tracking in frame ' sprintf('%d',f) '...']);
    ...
    % Motion Constraint
    zeta = 0.3; % weight of this constraint
    c_m = zeros(n*k,1);
    ...
    eta = 0.2; % weight of this constraint
    c_nm = zeros(n*k,1);
