function [cands, cands_percam] = sampleCandidates(N, k, num_cams, targs, homographies, invhomographies, sampling_plane, dx, dy, dataset)
    cands = cell(N,1);
            ...
            for gridx = -floor(sqrt(k)/2):floor(sqrt(k)/2)
              for gridy = -floor(sqrt(k)/2):floor(sqrt(k)/2)
                if strcmp(sampling_plane,'camera')
                    ...
                    feet = [cx + t_rect(3)/2; cy + t_rect(4)];
                    A = homographies.homographies(targs(t,1)).K * homographies.homographies(targs(t,1)).R;
                    b = homographies.homographies(targs(t,1)).KRC;
                    A = [A(:,1:2) [512-feet(1); 288-feet(2);-1]];
                    th = A\b;
                    cands{t}(i,:) = [cx cy t_rect(3:4) th(1:2)'];
                end
                i = i + 1;
              end
            end
