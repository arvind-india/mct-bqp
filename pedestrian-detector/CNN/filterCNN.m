function allDetections = filterCNN(allDetections, inplanes)
    ...
    for id=1:length(cameras)
        if use_GPU == 1
            filename = [gpu_results cameras{id} '.txt'];
        else
            filename = [cpu_results cameras{id} '.txt'];
        end
    ...
    delimiterIn = ' ';
    headerlinesIn = 1;
    systematic_wrong_detections{1} = importdata('/home/pedro/mct-bqp/campus2_data/FP-filter/alameda_CNN_false_positives.txt',delimiterIn,headerlinesIn); systematic_wrong_detections{1} = systematic_wrong_detections{1}.data;
    filename = fp_files(2);
    systematic_wrong_detections{2} = importdata('/home/pedro/mct-bqp/campus2_data/FP-filter/central_CNN_false_positives.txt',delimiterIn,headerlinesIn); systematic_wrong_detections{2} = systematic_wrong_detections{2}.data;
