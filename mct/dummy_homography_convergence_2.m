...
cam1_camdetections = [400.0 360.0;  700.0 700.0];
cam1_region = [4 796; 1022 798; 353 473; 142 431];

cam2_camdetections = [760.0 560.0; 640.0 640.0];
...
%------------------------------------------------------------------------------
% Compute initial detections in the ground plane
cam1_gpdetections = cam1_camdetections; cam2_gpdetections = cam2_camdetections;
for i = 1:size(cam1_gpdetections,1)
    cam1_gpdetections(:,i) = H(H1,cam1_camdetections);
end
for i = 1:size(cam2_gpdetections,1)
    cam2_gpdetections(:,i) = H(H2,cam2_camdetections);
end
...
N = 9;
%N = 5;
n_c2 = zeros(N+1,2); n_c2(1,:) = n_cam2_gpdetections';
n_c1 = zeros(N+1,2); n_c1(1,:) = n_cam1_gpdetections';
for reps = 1:N
    % Fix H1
    n_H2 = solve_homography(vertcat(repmat(cam2_region,rho_r,1), repmat(cam2_camdetections',rho_m,1)), ...
                          vertcat(repmat(n_cam2_gpregion,rho_r,1), repmat(n_cam1_gpdetections',rho_m,1)), 'svd');
    % Fix H2
    ...
    n_cam1_gpdetections = H(n_H1,cam1_camdetections); n_c1(reps+1,:) = n_cam1_gpdetections';
